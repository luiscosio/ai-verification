...
Prover material (the zkeys, 78 to 108 MB each, and the parameters file) is not in git; it is published as release assets or regenerated with `setup.sh`, in which case the verification keys here must be replaced too, because a new setup is a new key pair.
