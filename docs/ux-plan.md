...
AgentMetal hosts the workspace at https://178-156-248-181.sslip.io since September 15, 2026 (server `srv_2j7wytturoa3`, medium plan, lease to 2026-10-15). The site is public, including generation, since September 16. Pages publishing is disabled and the old Pages site was deleted on September 16. The older private artifact remains historical.
...
## September 14 figure revision

The jelly is gone. Fig. 01 now shows the thing the proof is about: a weight matrix as a slab of frosted glass tiles, drawn as one instanced mesh of 56 × 36 tiles. Each cycle one band of rows lifts out of the slab and turns lavender, a pulse of light travels along it while a point light follows, one output dot per row fills in at the edge, and a thin frame seals the band before it settles back. The band position changes every cycle. The frosted tiles stand for weights the verifier never reads; the single lit band stands for the one integer row group a proof covers. The caption says both in one line each.

The figure keeps every earlier constraint: decorative only, no proof data, a pause control, reduced-motion starts paused on the sealed frame, it stops while off-screen or hidden and while a verification is running, and the static SVG fallback (now the same slab and band) shows when WebGL is unavailable. The vendored Three.js bundle re-exports `BoxGeometry`, `InstancedMesh`, `Object3D`, `PointLight` and `DynamicDrawUsage` in addition to the earlier set; its digest in `site/vendor/manifest.json` changed accordingly and the supply-chain check passes. Verified in Chrome at 1400 px and headless Chromium at 1400 px and 390 px, plus the ten Chromium browser scenarios.
