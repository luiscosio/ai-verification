...
This is a public, single-workspace deployment. Every visitor shares one active job and its latest prompt/result. It is not a multi-user prover: a second visitor waits while a job runs. AgentMetal is the single hosting target for the website, model registry, browser verifier and prover. Opening a proof for verification stays in the visitor’s browser. Pages publishing is disabled; the old site remains temporarily available only until the replacement passes its live checks. Hosting does not change proof coverage.

The prover listens only on loopback inside the Caddy network namespace. Caddy provides HTTPS and compression; no Python port is published. The application independently checks the configured Host/Origin and its per-process request token. Prompts execute on this server, as the hosted UI states. Private temporary traces are removed by the existing job cleanup; the latest prompt/result stays in memory until replacement or restart.
...
4. Copy this directory to the server. Create a private `.env` file alongside it, containing `APP_REVISION` (the full reviewed public repository commit) and `PROVER_DOMAIN`.
...
6. Run `python3 deploy/agentmetal/check_hosted.py --url https://HOSTNAME --out /path/to/private/live-check.json` from the matching release checkout. This requires a publicly trusted certificate; only local container checks use a test certificate. Verify HTTPS, the discovery files and tags, token and origin enforcement, generation, proof download and independent verification. Reject changed sums, wrong origins and missing request tokens. Confirm that port 8789 is unreachable externally. Record the deployed commit, server ID and expiry; renew explicitly before expiry if the service should continue.
...
## September 16 public cutover

The login was removed at the operator's request: the site is public, including generation. The proxy now only terminates TLS, compresses responses, caps request bodies at 16 KB and defaults `Cache-Control` to `no-store`; the application still checks the configured Host and Origin and its per-process request token, and it still runs one job at a time with the existing timeouts and cleanup. Anyone can therefore run prompts on the server, about 50 seconds each, and a busy workspace answers the next visitor with a busy message. There is no rate limit beyond the single job.

The page gained discovery and preview metadata: description, canonical URL, robots, Open Graph and Twitter tags, JSON-LD, a favicon, `llms.txt`, `robots.txt`, `sitemap.xml` and a 1200 x 630 social image. The application serves these files next to the page and fills in its configured origin, so a domain change needs no rebuild. The hosted check now expects a public page with these files and no login.
