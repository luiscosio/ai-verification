...
(async () => {
    ...
    assert.match(elements["result-view"].textContent, /complete inference are not verified/);
    assert.match(elements["result-view"].textContent, /Proof accepted/);
    ...
    assert.match(coverage.textContent, /Each square is one tensor/);
    ...
    for (const id of ['models-view','model-search','catalog-summary']) elements[id] = new Element();
    context.DATA = {...data, manifests:catalogue, local_generation:[manifest.manifest_id]};
    context.renderModels();
    const onePointFive = elements['models-view'].children.find(el => el.textContent.includes('1.5B'));
    assert.ok(onePointFive);
    assert.match(onePointFive.textContent, /Cannot generate proofs/);
    assert.match(elements['models-view'].textContent, /Can generate proofs in the local workspace/);
    console.log(`${count} Groth16 regressions, 9 envelope cases and 4 application-renderer cases plus coverage and generation availability passed`);
    process.exit(0);
