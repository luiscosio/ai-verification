...
The row-proof matrix produces 18 proofs: three separated row groups per operation, across K=1024,1536,2048,3072 and two models. Each traced generation is paired with the same request without tracing; tokens are compared and elapsed capture overhead is recorded. It runs the shared registration/range policy and changed-sum/changed-group negative controls. The 1.5B path is a developer benchmark; it does not add that model to the local website picker. These proofs cover integer row groups, not the generated answer.
