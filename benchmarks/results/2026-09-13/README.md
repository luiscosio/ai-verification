...
A subsequent [clean-tree smoke run](clean-smoke/environment.json) at `390382246c3d6d532aadae93e46fac707de2df7c` records `dirty: false`, an empty Git status and the exact tree ID. Both Qwen3 cases (Arabic at eight threads and a longer prompt at two) passed generation and content/model identity checks; [raw results](clean-smoke/results.jsonl.gz) are retained. These two selected cases did not run repeat comparisons or replay. No source edits occurred during that run. The native binary was reused from the prior build, whose source is byte-identical after the PR-stack repair; its digest is recorded separately. This validates the new clean-start workflow and does not replace the original multi-model performance measurements.
